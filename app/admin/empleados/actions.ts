...
const employeeSchema = z.object({
  ...
  weeklyContractHours: z.coerce.number().min(0, "Debe ser un número positivo."),
...
export async function createEmployee(
  ...
  const { error: employeeError } = await admin.from("employees").insert({
    ...
    weekly_contract_hours: data.weeklyContractHours,
...
const updateSchema = z.object({
  ...
  weeklyContractHours: z.coerce.number().min(0),
